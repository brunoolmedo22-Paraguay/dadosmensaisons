...
METRIC_LABELS = {
    "ear_max_subsistema": "EAR máxima (MWmês)",
    "ear_verif_subsistema_mwmes": "EAR verificada (MWmês)",
    "ear_verif_subsistema_percentual": "EAR verificada (%)",
}
...
COLUMN_ALIASES = {
    ...
    "ear_max_subsistema": {
        "ear_max_subsistema",
        "ear_maxima_subsistema",
        "ear_max",
    },
    "ear_verif_subsistema_mwmes": {
        "ear_verif_subsistema_mwmes",
        "ear_verificada_subsistema_mwmes",
        "ear_verif_mwmes",
    },
    "ear_verif_subsistema_percentual": {
        "ear_verif_subsistema_percentual",
        "ear_verificada_subsistema_percentual",
        "ear_verif_percentual",
    },
}
...
def build_period_summary(
    ...
    """Agrega a série diária em base diária, mensal ou anual."""
    if granularity not in GRANULARITY_PERIOD_COLUMNS:
        raise ValueError(f"Discretização inválida: {granularity}")
    if daily.empty or "ear_data" not in daily.columns:
        return pd.DataFrame()

    data = daily.copy()
    data["ear_data"] = pd.to_datetime(data["ear_data"], errors="coerce")
    data = data.dropna(subset=["ear_data"])
    if start_date is not None:
        data = data.loc[data["ear_data"].ge(pd.Timestamp(start_date))]
    if end_date is not None:
        data = data.loc[data["ear_data"].lt(pd.Timestamp(end_date) + pd.Timedelta(days=1))]
    if data.empty:
        return pd.DataFrame()
    ...
    if granularity == "daily":
        data["__period_start"] = data["ear_data"].dt.floor("D")
    elif granularity == "monthly":
        data["__period_start"] = data["ear_data"].dt.to_period("M").dt.to_timestamp()
    else:
        data["__period_start"] = data["ear_data"].dt.to_period("Y").dt.to_timestamp()

    grouped = data.groupby("__period_start", sort=True, observed=True)
    means = grouped[metric_codes].mean()
    days = grouped["ear_data"].nunique().rename("Dias com dados")
...
def _process_sources(
    ...
    combined = combined.sort_values(
        ["ear_data", "__subsystem_key", "__file_order"], kind="stable"
    ).reset_index(drop=True)

    overlap_mask = combined.duplicated(
        subset=["__subsystem_key", "ear_data"], keep="last"
    )
    ...
    combined = combined.sort_values(
        ["ear_data", "__subsystem_key"], kind="stable"
    ).reset_index(drop=True)
...
def _load_single_parquet(
    ...
    warnings: list[str] = []
    frame["ear_data"] = pd.to_datetime(frame["ear_data"], errors="coerce").dt.floor("D")
    invalid_dates = int(frame["ear_data"].isna().sum())
    if invalid_dates:
    ...
    frame = frame.dropna(subset=["ear_data", "__subsystem_key", *METRIC_LABELS.keys()])
    outside_year = frame["ear_data"].dt.year.ne(year)
    ...
    period_start = frame["ear_data"].min().strftime("%d/%m/%Y")
    period_end = frame["ear_data"].max().strftime("%d/%m/%Y")
...
def _append_sin_series(data: pd.DataFrame) -> pd.DataFrame:
    ...
    sums = regional.groupby("ear_data", sort=True, observed=True).agg(
        ear_max_subsistema=("ear_max_subsistema", "sum"),
        ear_verif_subsistema_mwmes=("ear_verif_subsistema_mwmes", "sum"),
        __file_order=("__file_order", "max"),
    ).reset_index()
    denominator = sums["ear_max_subsistema"].replace(0, pd.NA)
    sums["ear_verif_subsistema_percentual"] = (
        sums["ear_verif_subsistema_mwmes"] / denominator * 100
    )
    sums["id_subsistema"] = "SIN"
    sums["nom_subsistema"] = "SIN"
    sums["__subsystem_key"] = "SIN"
    sums["__subsystem_label"] = "SIN"
    return pd.concat([data, sums[data.columns]], ignore_index=True, sort=False)
