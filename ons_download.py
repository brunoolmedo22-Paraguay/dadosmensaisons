...
DATASET_URL = "https://dados.ons.org.br/dataset/balanco-energia-subsistema"
REQUEST_TIMEOUT = (15, 120)
DOWNLOAD_CHUNK_SIZE = 1024 * 1024
MAX_PARQUET_SIZE = 50 * 1024 * 1024
USER_AGENT = (
    "Balanço-Mensal-SIN/2.0 "
...
PARQUET_FILENAME_PATTERN = re.compile(
    r"^BALANCO_ENERGIA_SUBSISTEMA_((?:19|20)\d{2})\.parquet$",
    flags=re.IGNORECASE,
)
    ...
    try:
        ...
        if not resources:
            raise ONSDownloadError(
                ...
                "foi encontrado. O portal pode ter alterado sua estrutura."
            )
        ...
        if declared_size:
            try:
                if int(declared_size) > MAX_PARQUET_SIZE:
                    raise ONSDownloadError(
                        "o arquivo excede o limite de segurança de 50 MB."
                    )
            except ValueError:
                pass
...
def _is_allowed_resource_url(url: str) -> bool:
    ...
    return (
        ...
        and PARQUET_FILENAME_PATTERN.fullmatch(
