...
PARQUET_FILENAME_PATTERN = re.compile(
    r"^ENA_DIARIO_SUBSISTEMA_((?:19|20)\d{2})\.parquet$",
    flags=re.IGNORECASE,
)
    ...
    try:
        ...
        if not resources:
            raise ONSDownloadError(
                ...
                "de ENA foi encontrado. O portal pode ter alterado sua estrutura."
            )
    ...
    try:
        ...
        for position, year in enumerate(requested_years, start=1):
            ...
            if url is None:
                ...
                filename = f"ENA_DIARIO_SUBSISTEMA_{year}.parquet"
